...
Hybrid-workflow для обхода задержки ProxyAPI с новыми моделями Claude. Skill живёт в `.claude/skills/lora-post-builder/SKILL.md` проекта (версионируется в git, переезжает с проектом на сервер) и применяется когда пишешь Claude'у в чате:
